<?php
    ...
    if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '領料管理', '查詢')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有領料資料的查詢權限，如需該功能的使用權限，請與管理者聯絡");
    ...
    $arrQryFld = ['jobort'=>"", 'mtrlapl'=>""]; //儲存淨化查詢條件
    $strStsMsg = ""; //儲存狀態欄訊息
    ...
    if(isset($_POST['query'])){ //按下"查詢"按鈕的處理動作
        ...
        $obj_stocks->SQLFrom = " from stocks s LEFT OUTER JOIN materials m ON(m.formstate=15 AND s.splrcode=m.splrcode AND s.mtrlcode=m.mtrlcode) ";
        $obj_stocks->SQLWhere .= " AND m.formstate = 15 AND s.formstate = 15 AND s.jobmth = '請領' ";
        $obj_stocks->SQLWhere .= isset($arrQryFld['jobort']) && mb_strlen($arrQryFld['jobort']) > 0 ? " AND s.jobort LIKE '%{$arrQryFld['jobort']}%' " : ""; //領料人
        ...
        // $obj_stocks->SQLOrderBy .= " s.formcode DESC ";
        $obj_stocks->SQLOrderBy .= isset($arrQryFld['sort']) ? " $arrQryFld[sort] " : '' ; //設定排序方式
        $htmlTags['html_sort'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'sort', 'attrName'=>'sort', 'attrTitle'=>'請輸入排序方式', 'optionTitle'=>'srtTitle', 'optionValue'=>'srtValue', 'default'=>'srtValue'), array(array('srtTitle'=>'作業日期欄位-由小到大排序', 'srtValue'=>'s.jobdate ASC'), array('srtTitle'=>'作業日期欄位-由大到小排序', 'srtValue'=>'s.jobdate DESC')), $arrQryFld['sort']); //排序方式
        ...
        $htmlQryResult = $obj_stocks->viewRcvMtrl($obj_stocks->rtnQryResults($obj_stocks->SQL));
        ...
        header("location: receivingMaterialEdit.php");
        exit();
        ...
        if (isset($_POST['discard'])) { //按下"註銷"按鈕的處理動作
            if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '領料管理', '註銷')) { //檢查使用者是否有使用權限
        ...
        $htmlTags['html_sort'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'sort', 'attrName'=>'sort', 'attrTitle'=>'請輸入排序方式', 'optionTitle'=>'srtTitle', 'optionValue'=>'srtValue'), array(array('srtTitle'=>'作業日期欄位-由小到大排序', 'srtValue'=>'s.jobdate ASC'), array('srtTitle'=>'作業日期欄位-由大到小排序', 'srtValue'=>'s.jobdate DESC')), "作業日期欄位-由大到小排序"); //排序方式
        ...
        $(document).ready(function(){
            ...
            $(":submit").click(function(){
                if($(this).val() == "編輯"){ //按下"編輯"按鈕
                    //$("#form1").attr("action", "receivingMaterialEdit.php"); //設定form表單的action屬性
                    msg = "開始編輯";
                    btn = "編輯";
...
echo <<<_html
    ...
    <main>
        ...
        <h4 class="text-secondary text-decoration-underline my-3"><b>查詢領料資料</b></h4>
        <div class="row justify-content-center mt-3">
            <input type="hidden" id="selFormCode" name="selFormCode" value="">
            <div class="row">
                <div class="col-sm-2">
                    <label for="applicant" class="form-label">領料人：</label>
                    <input type="text" class="form-control" id="jobort" name="jobort" value="{$arrQryFld['jobort']}" title="請輸入領料人" placeholder="請輸入領料人">
                </div>
                <div class="col-sm-2">
                    <label for="supplier_telephone" class="form-label">品項名稱：</label>
            ...
            <table class="table caption-top table-striped table-hover my-5">
                <caption><h4><b>領料清單</b></h4></caption>
                <thead class="">
                    <tr>
                        <th class="col-sm-1 text-center">功能</th><th class="col-sm-1">作業日期</th><th class="col-sm-1">作業異動者</th><th class="col-sm-1">供應商代碼</th><th class="col-sm-1">供應商名稱</th><th class="col-sm-1">品項代碼</th><th class="col-sm-2">品項名稱</th><th class="col-sm-1 text-center">品項數量</th><th class="col-sm-1 text-center">品項單價</th><th class="col-sm-1 text-center">品項總金額</th><th class="col-sm-1 text-center">安全存量</th>
                    </tr>
                </thead>
