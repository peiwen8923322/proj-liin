<?php
    ...
    if (!$obj_pms->isOwnPmsByEmpformcode($_SESSION['login_emp']['formcode'], '刷卡管理', '查詢')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有刷卡管理的查詢權限，如需該功能的使用權限，請與管理者聯絡");
    ...
    $tbl = array(); // 儲存不同的參考檔
    $arrQryFld = ['year'=>"", 'begindate'=>"", 'enddate'=>"", 'isnormality'=>"", 'recdsperpage'=>'']; // 儲存淨化查詢條件
    $strStsMsg = ""; //儲存狀態欄訊息
    ...
    if(isset($_POST['query'])){ //按下"查詢"按鈕的處理動作
        ...
        // $obj_clockin->SQLSelect .= ", year(CURRENT_DATE()) - year(takeofcdate) AS 'seniority' ";
        $obj_clockin->SQLWhere .= " AND formstate = 15 AND empformcode = '{$_SESSION['login_emp']['formcode']}' ";
        
        $obj_clockin->SQLWhere .= isset($arrQryFld['year']) && mb_strlen($arrQryFld['year']) > 0 ? " AND year = $arrQryFld[year] " : ""; // 年度(西元年)
        $htmlTags['year'] = $obj_form->viewHTMLSTSglVal(array('attrId'=>'year', 'attrName'=>'year', 'attrTitle'=>'請選擇年度'), array(date("Y", time())-4, date("Y", time())-3, date("Y", time())-2, date("Y", time())-1, date("Y", time())), $arrQryFld['year'], false); // 年度(西元年)
...
echo <<<_html
    ...
    <main>
        ...
        <h4 class="text-secondary text-decoration-underline my-3"><b>查詢刷卡資料</b></h4>
        <div class="row justify-content-center mt-3">
            <input type="hidden" id="selFormCode" name="selFormCode" value="">
            <div class="row">
                <div class="col-sm-2"><label for="year" class="form-label">年度(西元年)：</label>$htmlTags[year]</div>
                <div class="col-sm-2">
