<?php
    ...
    */
    ...
    if (isset($_POST['submit'])) { //按下"送出 / 暫存"按鈕的處理動作
        ...
        $htmlTags['empapl'] = $_SESSION['login_emp']['empapl']; // 外出員工
        $htmlTags['empcode'] = $_SESSION['login_emp']['empcode']; // 員工編號
        $htmlTags['year'] = date("Y", time()); // 年度
        $htmlTags['egrersn'] = ""; // 外出事由
        $htmlTags['begindate'] = date("Y-m-d H:i", time()); // 外出啟始日
        $htmlTags['enddate'] = date("Y-m-d H:i", time()); // 外出截止日
    }
...
echo <<<_html
    ...
    <main>
        ...
        <h4 class="text-secondary text-decoration-underline my-3"><b>建立員工外出資料</b></h4>
        <div class="row">
            <div class="col-10">
                <div class="row">
