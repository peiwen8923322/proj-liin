<?php
    ...
    */
    ...
    if(isset($_POST['query']) || isset($_POST['prt'])){ //按下"查詢"按鈕的處理動作 OR 按下"列印"按鈕的處理動作
        ...
        if ((isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) && (isset($arrQryFld['enddate']) && mb_strlen($arrQryFld['enddate']) > 0)) { // 外出起始日 + 外出截止日
        ...
        } elseif (isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) { // 外出起始日
...
echo <<<_html
    ...
    <main>
        <div class="row"><h5 class="alert alert-success text-primary fw-bold">狀態列：$strStsMsg</h5></div>
            <h4 class="text-secondary text-decoration-underline my-3"><b>統計員工外出資料</b></h4>
            <div class="row justify-content-center mt-3">
                ...
                <table class="table caption-top table-striped table-hover my-5">
                    <caption><h4><b>統計員工外出資料清單(主任已簽核)</b></h4></caption>
                    <thead class="">
                        <tr>
                            <th class="">機構</th><th class="text-center">員工</th><th class="text-center">年度</th><th class="text-center">外出次數</th><th class="text-center">外出時數</th>
                        </tr>
                    </thead>
