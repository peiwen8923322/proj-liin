<?php
    ...
    if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '庫存品項管理', '編輯')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有庫存品項資料的編輯權限，如需該功能的使用權限，請與管理者聯絡");
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == "確定") {
        ...
        //執行SQL
        $obj_materials->Update($arrNewFormVal, $tbl);
        ...
        $obj_materials->SQLSelect = "SELECT * ";
        $obj_materials->SQLFrom = $_SESSION['SQL']['From'];
        $obj_materials->SQLWhere = " WHERE 1 AND formcode = '$_SESSION[selFormCode]'";
        $obj_materials->SQL = $obj_materials->SQLSelect.$obj_materials->SQLFrom.$obj_materials->SQLWhere;
        $arrCurRecord = $obj_materials->rtnQryRecord($obj_materials->SQL); //取得目前的編輯記錄
        ...
        $htmlTags['html_mtrlcode'] = $arrCurRecord['mtrlcode']; //品項代碼
        $htmlTags['html_mtrlapl'] = $arrCurRecord['mtrlapl']; //品項名稱
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(":button").click(function() {
                if ($(this).val() == "關閉") {
                    location.assign("./materialsQuery.php?action=cancel");
                }else if ($(this).val() == "登出") {
                    msg = "你已經登出系統";
                    btn = "登出";
...
echo <<<_html
    ...
    <main>
        ...
        <div class="container-fluid">
            <h4 class="text-secondary text-decoration-underline my-3"><b>編輯品項資料</b></h4>
