<?php
    ...
    if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '盤點管理', '編輯')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有領料資料的編輯權限，如需該功能的使用權限，請與管理者聯絡");
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == "確定") {
        ...
        //品項單價清單
        $htmlTags['html_mtrlprclist'] = count($obj_stocks->getListByMtrlPrc($arrCurRecord['splrcode'], $arrCurRecord['mtrlcode'])) > 0 ? $obj_form->viewHTMLSelectTag(array('attrId'=>'mtrlprclist', 'attrName'=>'mtrlprclist', 'attrTitle'=>'請選擇品項單價，如果沒有合適的品項單價，請在右欄自行輸入。', 'optionTitle'=>'mtrlprc', 'optionValue'=>'mtrlprc'), $obj_stocks->getListByMtrlPrc($arrCurRecord['splrcode'], $arrCurRecord['mtrlcode']), $arrCurRecord['mtrlprc']) : $obj_form->viewHTMLSelectTag(array('attrId'=>'mtrlprclist', 'attrName'=>'mtrlprclist', 'attrTitle'=>'請選擇品項單價，如果沒有合適的品項單價，請在右欄自行輸入。', 'optionTitle'=>'mtrlprc', 'optionValue'=>'mtrlprc'), array(), null, true) ; //如果品項單價不存在, 預設值為空白選項
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(":button").click(function() {
                if ($(this).val() == "關閉") {
                    location.assign("./checkStocksQuery.php?action=cancel");
                }else if ($(this).val() == "登出") {
                    msg = "你已經登出系統";
                    btn = "登出";
...
echo <<<_html
    ...
    <main>
        ...
        <div class="container-fluid">
            <h4 class="text-secondary text-decoration-underline my-3"><b>編輯盤點資料</b></h4>
