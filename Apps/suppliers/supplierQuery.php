<?php
    ...
    if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '供應商管理', '查詢')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有供應商資料的查詢權限，如需該功能的使用權限，請與管理者聯絡");
        $obj_form->js_goURL(INDEXPAGE); //返回首頁
        exit();
    }
    $obj_suppliers = new cls_suppliers; //品項檔
    
    $htmlTags = array(); //Render HTML
    $SQL = "";
    $htmlQryResult = ""; //顯示查詢結果HTML Tag
    $htmlPaging = ""; //顯示查詢分頁HTML Tag
    $arrQryFld = ['splrapl'=>"", 'splrlia'=>"", 'sort'=>"", 'recdsperpage'=>""]; //儲存淨化查詢條件
    $strStsMsg = ""; //儲存狀態欄訊息
    ...
    if(isset($_POST['query'])){ //按下"查詢"按鈕的處理動作
        ...
        //$obj_suppliers->SQLSelect .= ", year(CURRENT_DATE()) - year(takeofcdate) AS 'seniority' ";
        $obj_suppliers->SQLWhere .= " AND formstate = 15 ";
        $obj_suppliers->SQLWhere .= isset($arrQryFld['splrapl']) && mb_strlen($arrQryFld['splrapl']) > 0 ? " AND splrapl LIKE '%{$arrQryFld['splrapl']}%' " : ""; //供應商名稱
        $obj_suppliers->SQLWhere .= isset($arrQryFld['splrlia']) && mb_strlen($arrQryFld['splrlia']) > 0 ? " AND splrlia LIKE '%{$arrQryFld['splrlia']}%' " : ""; //供應商聯絡人
        //$obj_suppliers->SQLWhere .= isset($arrQryFld['mobilephone']) && mb_strlen($arrQryFld['mobilephone']) > 0 ? " AND mobilephone LIKE '%{$arrQryFld['mobilephone']}%' " : ""; //手機號碼欄
        $obj_suppliers->SQLOrderBy .= " splrcode ASC " ; //設定排序方式
        //$obj_suppliers->SQLOrderBy .= isset($arrQryFld['sort']) ? " $arrQryFld[sort] " : '' ; //設定排序方式
        ...
        //統計分頁訊息
        $obj_suppliers->int_records_per_page = $arrQryFld['recdsperpage']; //設定每頁筆數
        $obj_suppliers->calTotalPages($obj_suppliers->rtnQryField("SELECT COUNT(*) AS 'cnt_recds' ".$obj_suppliers->SQLFrom.$obj_suppliers->SQLWhere));
        $htmlPaging = $obj_form->viewPaging($obj_suppliers->int_total_records, $obj_suppliers->int_total_pages, $obj_suppliers->int_current_page); //顯示查詢分頁HTML Tag

        //傳回查詢結果
        $obj_suppliers->SQLlimit = " LIMIT $obj_suppliers->intStartPos, $obj_suppliers->int_records_per_page";
        $obj_suppliers->SQL = $obj_suppliers->SQLSelect.$obj_suppliers->SQLFrom.$obj_suppliers->SQLWhere.$obj_suppliers->SQLOrderBy.$obj_suppliers->SQLlimit;
        $htmlQryResult = $obj_suppliers->viewQry($obj_suppliers->rtnQryResults($obj_suppliers->SQL));

        //Save Session
        $_SESSION['arrQryFld'] = $arrQryFld; //儲存查詢條件
        $_SESSION['SQL']['Select'] = $obj_suppliers->SQLSelect;
        $_SESSION['SQL']['From'] = $obj_suppliers->SQLFrom;
        $_SESSION['SQL']['Where'] = $obj_suppliers->SQLWhere;
        $_SESSION['SQL']['OrderBy'] = $obj_suppliers->SQLOrderBy;
        $_SESSION['SQL']['CurPage'] = $obj_suppliers->int_current_page; //儲存目前頁數
        ...
        if (isset($_POST['discard'])) { //按下"註銷"按鈕的處理動作
            if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '供應商管理', '註銷')) { //檢查使用者是否有使用權限
                $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有供應商資料的註銷權限，如需該功能的使用權限，請與管理者聯絡");
                $obj_form->js_goURL(INDEXPAGE); //返回首頁
                exit();
            }
            $obj_suppliers->discard($_POST['selFormCode']); //註銷記錄
        }
        
        //取得分頁條件
        $arrQryFld = $_SESSION['arrQryFld']; //取得查詢條件
        $obj_suppliers->SQLSelect = $_SESSION['SQL']['Select'];
        $obj_suppliers->SQLFrom = $_SESSION['SQL']['From'];
        $obj_suppliers->SQLWhere = $_SESSION['SQL']['Where'];
        $obj_suppliers->SQLOrderBy = $_SESSION['SQL']['OrderBy'];
        //$htmlTags['html_sort'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'sort', 'attrName'=>'sort', 'attrTitle'=>'請輸入排序方式', 'optionTitle'=>'srtTitle', 'optionValue'=>'srtValue', 'default'=>'srtValue'), array(array('srtTitle'=>'品項代碼欄位-由小到大排序', 'srtValue'=>'mtrlcode ASC'), array('srtTitle'=>'品項代碼欄位-由大到小排序', 'srtValue'=>'mtrlcode DESC')), $arrQryFld['sort']); //排序方式
        $htmlTags['html_recdsperpage'] = $obj_form->viewHTMLPagingTag(array('attrId'=>'recdsperpage', 'attrName'=>'recdsperpage', 'attrTitle'=>'請輸入每頁顯示筆數', 'optionTitle'=>'srtTitle', 'optionValue'=>'srtValue'), null, $arrQryFld['recdsperpage']); //每頁顯示筆數
        //$htmlTags['html_recdsperpage'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'recdsperpage', 'attrName'=>'recdsperpage', 'attrTitle'=>'請輸入每頁顯示筆數', 'optionTitle'=>'srtTitle', 'optionValue'=>'srtValue'), array(array('srtTitle'=>'50', 'srtValue'=>'50'), array('srtTitle'=>'100', 'srtValue'=>'100'), array('srtTitle'=>'250', 'srtValue'=>'250'), array('srtTitle'=>'500', 'srtValue'=>'500')), $arrQryFld['recdsperpage']); //每頁顯示筆數
        ...
        if (isset($_POST['paging'])) { //分頁
            $obj_suppliers->int_current_page = $_POST['CurPage'];
            switch ($_POST['paging']) {
                case '|< 第一頁':
                    $obj_suppliers->int_current_page = 1;
                    break;
                case '<< 上一頁':
                    $obj_suppliers->int_current_page--;
                    break;
                case '下一頁 >>':
                    $obj_suppliers->int_current_page++;
                    break;
                case '最後頁 >|':
                    $obj_suppliers->int_current_page = $obj_suppliers->int_total_pages;
                    break;
                default:
                    # code...
                    break;
            ...
            $obj_suppliers->int_current_page = $_SESSION['SQL']['CurPage']; //取得目前頁數
        }
        $htmlPaging = $obj_form->viewPaging($obj_suppliers->int_total_records, $obj_suppliers->int_total_pages, $obj_suppliers->int_current_page); //顯示查詢分頁HTML Tag

        //傳回查詢結果
        $obj_suppliers->intStartPos = ($obj_suppliers->int_current_page - 1) < 0 ? 0 : ($obj_suppliers->int_current_page - 1) * $obj_suppliers->int_records_per_page ; //資料註銷後, 檢查啟始記錄是否小於0
        $obj_suppliers->SQLlimit = " LIMIT $obj_suppliers->intStartPos,  $obj_suppliers->int_records_per_page";
        $obj_suppliers->SQL = $obj_suppliers->SQLSelect.$obj_suppliers->SQLFrom.$obj_suppliers->SQLWhere.$obj_suppliers->SQLOrderBy.$obj_suppliers->SQLlimit;
        $htmlQryResult = $obj_suppliers->viewQry($obj_suppliers->rtnQryResults($obj_suppliers->SQL));
    } else { //第一次執行時的處理動作
    ...
    if (isset($_SESSION['error'])) { //檢查是否有錯誤訊息
        ...
        $strStsMsg = "資料查詢完成, 總筆數：$obj_suppliers->int_total_records";
        ...
        $(document).ready(function(){
            ...
            $(":submit").click(function(){
                if($(this).val() == "編輯"){ //按下"編輯"按鈕
                    // $("#form1").attr("action", "supplierEdit.php"); //設定form表單的action屬性
                    msg = "開始編輯";
                    btn = "編輯";
...
echo <<<_html
    ...
    <main>
        ...
        <div class="container-fluid">
            <h4 class="text-secondary text-decoration-underline my-3"><b>查詢供應商資料</b></h4>
            <div class="row justify-content-center mt-3">
                <input type="hidden" id="selFormCode" name="selFormCode" value="">
                <div class="row">
                    <div class="col-3">
                        <label for="splrapl" class="form-label">供應商名稱：</label>
                        <input type="text" class="form-control" id="splrapl" name="splrapl" value="{$arrQryFld['splrapl']}" title="請輸入供應商名稱" placeholder="請輸入供應商名稱" autofocus>
                    </div>
                    <div class="col-3">
                        <label for="splrlia" class="form-label">供應商聯絡人：</label>
                        <input type="text" class="form-control" id="splrlia" name="splrlia" value="{$arrQryFld['splrlia']}" title="請輸入供應商聯絡人" placeholder="請輸入供應商聯絡人">
                    </div>
                    <div class="col-3">
                ...
                <div class="row  justify-content-center mt-2">
                    <input type="submit" class="col-1 btn btn-primary" id="query" name="query" value="查詢">&nbsp;&nbsp;<input type="reset" class="col-1 btn btn-outline-primary" value="清除">
                </div>
                
                <table class="table caption-top table-striped table-hover my-5">
                    <caption><h4><b>供應商清單</b></h4></caption>
                    <thead class="">
                        <tr>
                            <th>功能</th><th class="text-center">供應商代碼</th><th>供應商名稱</th><th class="text-center">統一編號</th><th class="text-center">供應商電話號碼</th><th class="text-center">供應商傳真號碼</th><th class="text-center">聯絡人</th><th class="text-center">手機號碼</th>
                        </tr>
                    </thead>
