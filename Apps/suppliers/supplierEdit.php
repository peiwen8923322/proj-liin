<?php
    ...
    if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '供應商管理', '編輯')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有供應商資料的編輯權限，如需該功能的使用權限，請與管理者聯絡");
        $obj_form->js_goURL(INDEXPAGE); //返回首頁
        exit();
    }
    $obj_suppliers = new cls_suppliers; //耗材檔
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == "確定") {
        ...
        //執行SQL
        $obj_suppliers->Update($arrNewFormVal);
        ...
        $obj_suppliers->SQLSelect = "SELECT * ";
        $obj_suppliers->SQLFrom = $_SESSION['SQL']['From'];
        $obj_suppliers->SQLWhere = " WHERE 1 AND formcode = '$_SESSION[selFormCode]'";
        $obj_suppliers->SQL = $obj_suppliers->SQLSelect.$obj_suppliers->SQLFrom.$obj_suppliers->SQLWhere;
        $arrCurRecord = $obj_suppliers->rtnQryRecord($obj_suppliers->SQL); //取得目前的編輯記錄
        
        //Render HTML
        $htmlTags['html_splrcode'] = $arrCurRecord['splrcode']; //供應商代碼
        $htmlTags['html_splrapl'] = $arrCurRecord['splrapl']; //供應商名稱
        $htmlTags['html_splrunicode'] = $arrCurRecord['splrunicode']; //統一編號
        $htmlTags['html_splrtel'] = $arrCurRecord['splrtel']; //供應商電話號碼
        $htmlTags['html_splrfax'] = $arrCurRecord['splrfax']; //供應商傳真號碼
        $htmlTags['html_splrlia'] = $arrCurRecord['splrlia']; //聯絡人
        $htmlTags['html_splrmbl'] = $arrCurRecord['splrmbl']; //手機號碼
        $htmlTags['html_splremail'] = $arrCurRecord['splremail']; //聯絡人Email
        $htmlTags['html_splraddr'] = $arrCurRecord['splraddr']; //供應商地址
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(":button").click(function() {
                if ($(this).val() == "關閉") {
                    location.assign("./supplierQuery.php?action=cancel");
                }else if ($(this).val() == "登出") {
                    msg = "你已經登出系統";
                    btn = "登出";
...
echo <<<_html
    ...
    <main>
        ...
        <div class="container-fluid">
            <h4 class="text-secondary text-decoration-underline my-3"><b>編輯供應商資料</b></h4>
