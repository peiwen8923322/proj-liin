<?php
    ...
    if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '進貨管理', '查詢')) { //檢查使用者是否有使用權限
        $obj_form->js_alert("使用者：[{$_SESSION['login_emp']['empapl']}]沒有進貨資料的查詢權限，如需該功能的使用權限，請與管理者聯絡");
    ...
    if(isset($_POST['query'])){ //按下"查詢"按鈕的處理動作
        ...
        $obj_stocks->SQLWhere .= " AND m.formstate = 15 AND s.formstate = 15 AND s.jobmth = '進貨' ";
        $obj_stocks->SQLWhere .= isset($arrQryFld['jobort']) && mb_strlen($arrQryFld['jobort']) > 0 ? " AND s.jobort LIKE '%{$arrQryFld['jobort']}%' " : ""; //進貨人欄
        ...
        header("location: purchaseOrdersEdit.php");
        exit();
        ...
        if (isset($_POST['discard'])) { //按下"註銷"按鈕的處理動作
            if (!$obj_pms->isOwnPms($_SESSION['login_emp']['empapl'], '進貨管理', '註銷')) { //檢查使用者是否有使用權限
        ...
        $(document).ready(function(){
            ...
            $(":submit").click(function(){
                if($(this).val() == "編輯"){ //按下"編輯"按鈕
                    //$("#form1").attr("action", "purchaseOrdersEdit.php"); //設定form表單的action屬性
                    msg = "開始編輯";
                    btn = "編輯";
...
echo <<<_html
    ...
    <main>
        ...
        <div class="container-fluid">
            <h4 class="text-secondary text-decoration-underline my-3"><b>查詢進貨資料</b></h4>
            <div class="row justify-content-center mt-3">
                <input type="hidden" id="selFormCode" name="selFormCode" value="">
                <div class="row">
                    <div class="col-3">
                        <label for="jobort" class="form-label">進貨人：</label>
                        <input type="text" class="form-control" id="jobort" name="jobort" value="{$arrQryFld['jobort']}" title="請輸入進貨人" placeholder="請輸入進貨人">
                    </div>
                    <div class="col-3">
                ...
                <table class="table caption-top table-striped table-hover my-5">
                    <caption><h4><b>進貨清單</b></h4></caption>
                    <thead class="">
                        <tr>
