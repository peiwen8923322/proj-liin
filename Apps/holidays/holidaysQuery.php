<?php
    ...
    if(isset($_POST['query']) || isset($_POST['prt'])){ // 按下"查詢"按鈕的處理動作 OR 按下"列印"按鈕的處理動作
        ...
        $obj_holiday->SQLWhere .= " AND formstate = 15 AND empcode = '{$_SESSION['login_emp']['empcode']}' ";
        
        $obj_holiday->SQLWhere .= isset($arrQryFld['year']) && mb_strlen($arrQryFld['year']) > 0 ? " AND year = '{$arrQryFld['year']}' " : ""; //年度
        ...
        if ((isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) && (isset($arrQryFld['enddate']) && mb_strlen($arrQryFld['enddate']) > 0)) { //請假啟始日 + 請假截止日
            $obj_holiday->SQLWhere .= " AND begindate >= '{$arrQryFld['begindate']}' AND enddate <= '{$arrQryFld['enddate']}' ";
        ...
        $obj_holiday->calTotalPages($obj_holiday->rtnQryField("SELECT COUNT(*) AS cnt_hlds ".$obj_holiday->SQLFrom.$obj_holiday->SQLWhere)); // 計算總頁數
        ...
        $obj_holiday->SQL = $obj_holiday->SQLSelect.$obj_holiday->SQLFrom.$obj_holiday->SQLWhere.$obj_holiday->SQLOrderBy.$obj_holiday->SQLlimit;
        $htmlQryResult = $obj_holiday->viewQry($obj_holiday->rtnQryResults($obj_holiday->SQL), $tbl);
        ...
        // 統計分頁訊息
        $obj_holiday->int_records_per_page = $arrQryFld['recdsperpage']; // 設定每頁筆數
        $obj_holiday->calTotalPages($obj_holiday->rtnQryField("SELECT COUNT(*) AS 'cnt_recds' ".$obj_holiday->SQLFrom.$obj_holiday->SQLWhere)); // 計算總頁數
        ...
        // 傳回查詢結果
        $obj_holiday->intStartPos = ($obj_holiday->int_current_page - 1) < 0 ? 0 : ($obj_holiday->int_current_page - 1) * $obj_holiday->int_records_per_page ; // 資料註銷後, 檢查啟始記錄是否小於0
        $obj_holiday->SQLlimit = " LIMIT $obj_holiday->intStartPos,  $obj_holiday->int_records_per_page";
        $obj_holiday->SQL = $obj_holiday->SQLSelect.$obj_holiday->SQLFrom.$obj_holiday->SQLWhere.$obj_holiday->SQLOrderBy.$obj_holiday->SQLlimit;
        $htmlQryResult = $obj_holiday->viewQry($obj_holiday->rtnQryResults($obj_holiday->SQL), $tbl);
    } else { //第一次執行時的處理動作
...
echo <<<_html
    ...
    <main>
        ...
        <h4 class="text-secondary text-decoration-underline my-3"><b>查詢員工請假資料</b></h4>
        <div class="row justify-content-center mt-3">
            ...
            <table class="table caption-top table-striped table-hover my-5">
                ...
                <thead class="">
                    <tr>
                        <th class="col-sm-1 text-center">功能</th><th class="">審核狀態</th><th class="text-center">年度</th><th class="text-center">員工</th><th class="text-center">代理人</th><th class="text-center">假別</th><th class="col-sm-3">請假事由</th><th class="text-center">假單送出時間</th><th class="text-center">請假啟始日</th><th class="text-center">請假截止日</th><th class="text-center">請假天數</th><th class="text-center">請假時數</th>
                    </tr>
                </thead>
