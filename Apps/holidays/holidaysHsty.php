<?php
    ...
    $tbl = array(); //儲存不同的參考檔
    $arrQryFld = ['year'=>"", 'empapl'=>'', 'empcode'=>'', 'begindate'=>"", 'enddate'=>"", 'recdsperpage'=>'']; //儲存淨化查詢條件
    $strStsMsg = ""; //儲存狀態欄訊息
    ...
    if(isset($_POST['query']) || isset($_POST['prt'])){ //按下"查詢"按鈕的處理動作 OR 按下"列印"按鈕的處理動作
        ...
        $htmlQryResult = $obj_holiday->viewHstyQry($obj_holiday->rtnQryResults($obj_holiday->SQL), $tbl);
