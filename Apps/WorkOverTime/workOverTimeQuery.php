<?php
    ...
    */
    ...
    if(isset($_POST['query'])){ //按下"查詢"按鈕的處理動作
        ...
        // $obj_egress->SQLSelect .= ", year(CURRENT_DATE()) - year(takeofcdate) AS 'seniority' ";
        $obj_egress->SQLWhere .= " AND formstate = 15 AND cls = '加班' AND empformcode = '{$_SESSION['login_emp']['formcode']}' ";
        ...
        if ((isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) && (isset($arrQryFld['enddate']) && mb_strlen($arrQryFld['enddate']) > 0)) { //外出啟始日 + 外出截止日
            $obj_egress->SQLWhere .= " AND begindate >= '$arrQryFld[begindate] 00:00:00' AND enddate <= '$arrQryFld[enddate] 23:59:59' ";
        } elseif (isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) { // 外出啟始日
            $obj_egress->SQLWhere .= " AND begindate >= '$arrQryFld[begindate] 00:00:00' ";
        } elseif (isset($arrQryFld['enddate']) && mb_strlen($arrQryFld['enddate']) > 0) { // 外出截止日
            $obj_egress->SQLWhere .= " AND enddate <= '$arrQryFld[enddate] 23:59:59' ";
        }
        $obj_egress->SQLOrderBy .= " begindate DESC, applydate DESC ";
        ...
        $obj_egress->calTotalPages($obj_egress->rtnQryField("SELECT COUNT(*) AS cnt_recd ".$obj_egress->SQLFrom.$obj_egress->SQLWhere)); // 計算總頁數
        ...
        $htmlQryResult = $obj_egress->viewQry($obj_egress->rtnQryResults($obj_egress->SQL), $tbl);
        ...
        header("location: workOverTimeEdit.php");
        exit();
        ...
        // 統計分頁訊息
        $obj_egress->int_records_per_page = $arrQryFld['recdsperpage']; // 設定每頁筆數
        $obj_egress->calTotalPages($obj_egress->rtnQryField("SELECT COUNT(*) AS 'cnt_recds' ".$obj_egress->SQLFrom.$obj_egress->SQLWhere)); // 計算總頁數
...
echo <<<_html
    ...
    <main>
        ...
        <div class="row justify-content-center mt-3">
            ...
            <div class="row">
                <div class="col-sm-2"><label for="year" class="form-label">年度(西元年)：</label>$htmlTags[html_year]</div>
                <div class="col-sm-2">
                    <label for="begindate" class="form-label">加班啟始日：</label>
                    <input type="date" class="form-control" id="begindate" name="begindate" value="{$arrQryFld['begindate']}" placeholder="請輸入加班啟始日" title="請輸入加班啟始日">
                </div>
                <div class="col-sm-2">
            ...
            <table class="table caption-top table-striped table-hover my-5">
                ...
                <thead class="">
                    <tr>
                        <th class="col-sm-1 text-center">功能</th><th class="">審核狀態</th><th class="">機構</th><th class="text-center">員工</th><th class="text-center">年度</th><th class="col-sm-3">加班事由</th><th class="text-center">加班啟始日</th><th class="text-center">加班截止日</th><th class="text-center">加班時數</th><th class="text-center">加班者簽核時間</th>
                    </tr>
                </thead>
