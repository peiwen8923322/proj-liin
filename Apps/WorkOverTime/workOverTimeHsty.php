<?php
    ...
    */
    ...
    if(isset($_POST['query']) || isset($_POST['prt'])){ //按下"查詢"按鈕的處理動作 OR 按下"列印"按鈕的處理動作
        ...
        $obj_egress->SQLWhere .= " AND formstate = 15 AND cls = '加班'  ";
        ...
        $htmlTags['depts'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'deptspk', 'attrName'=>'deptspk', 'attrTitle'=>'請選擇機構', 'optionTitle'=>'cmpapl', 'optionValue'=>'formcode', 'default'=>'formcode'), $obj_depts->getList(), $arrQryFld['deptspk'], false); // 機構
        $obj_egress->SQLWhere .= isset($arrQryFld['empapl']) && mb_strlen($arrQryFld['empapl']) > 0 ? " AND empapl LIKE '%$arrQryFld[empapl]%' " : ""; // 員工
        if ((isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) && (isset($arrQryFld['enddate']) && mb_strlen($arrQryFld['enddate']) > 0)) { // 加班啟始日 + 加班截止日
            $obj_egress->SQLWhere .= " AND begindate >= '$arrQryFld[begindate]' AND enddate <= '$arrQryFld[enddate]' ";
        } elseif (isset($arrQryFld['begindate']) && mb_strlen($arrQryFld['begindate']) > 0) { // 加班啟始日
        ...
        if (isset($arrQryFld['isSigned']) && $arrQryFld['isSigned'] == '是') {
            $obj_egress->SQLWhere .= " AND frmformcode = '2023010017' ";
        ...
        $obj_egress->SQLOrderBy .= " year DESC, begindate DESC, cmpcode, empcode ";
        ...
        $obj_egress->SQL = $obj_egress->SQLSelect.$obj_egress->SQLFrom.$obj_egress->SQLWhere.$obj_egress->SQLOrderBy.$obj_egress->SQLlimit;
        $htmlQryResult = $obj_egress->viewHstyQry($obj_egress->rtnQryResults($obj_egress->SQL), $tbl);
        ...
        if (isset($_POST['prt'])) { //按下"列印"按鈕的處理動作
            $obj_form->js_openWindow('workOverTimeHstyRpt.php');
        }
        ...
        $htmlTags['html_year'] = $obj_form->viewHTMLSTSglVal(array('attrId'=>'year', 'attrName'=>'year', 'attrTitle'=>'請選擇年度'), array(date("Y", time())-2, date("Y", time())-1, date("Y", time()), date("Y", time())+1), $arrQryFld['year'], true); // 年度(西元年)
        $htmlTags['depts'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'deptspk', 'attrName'=>'deptspk', 'attrTitle'=>'請選擇機構', 'optionTitle'=>'cmpapl', 'optionValue'=>'formcode', 'default'=>'formcode'), $obj_depts->getList(), $arrQryFld['deptspk'], false); // 機構
        $htmlTags['html_recdsperpage'] = $obj_form->viewHTMLPagingTag(array('attrId'=>'recdsperpage', 'attrName'=>'recdsperpage', 'attrTitle'=>'請輸入每頁顯示筆數', 'optionTitle'=>'srtTitle', 'optionValue'=>'srtValue'), null, $arrQryFld['recdsperpage']); //每頁顯示筆數
        ...
        $htmlTags['depts'] = $obj_form->viewHTMLSelectTag(array('attrId'=>'deptspk', 'attrName'=>'deptspk', 'attrTitle'=>'請選擇機構', 'optionTitle'=>'cmpapl', 'optionValue'=>'formcode'), $obj_depts->getList(), null, false); //機構
...
echo <<<_html
    ...
    <main>
        <div class="row"><h5 class="alert alert-success text-primary fw-bold">狀態列：$strStsMsg</h5></div>
            <h4 class="text-secondary text-decoration-underline my-3"><b>查詢加班歷史資料</b></h4>
            <div class="row justify-content-center mt-3">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <label for="begindate" class="form-label">員工：</label>
                ...
                <table class="table caption-top table-striped table-hover my-5">
                    <caption><h4><b>加班歷史資料清單</b></h4></caption>
                    <thead class="">
                        <tr>
                            <th class="text-center">審核狀態</th><th class="">機構</th><th class="text-center">員工</th><th class="text-center">年度</th><th class="col-sm-3">加班事由</th><th class="text-center">加班起始日</th><th class="text-center">加班截止日</th><th class="text-center">加班時數</th><th class="text-center">加班者簽核時間</th>
                        </tr>
                    </thead>
